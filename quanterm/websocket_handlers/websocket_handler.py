...
class WebsocketHandler:
    ...
    async def subscribe(self, pair: str, stream_type: StreamType, out_queue: asyncio.Queue):
        stream_id = f"{pair}@{stream_type}"
        if stream_id in self._active_streams:
            print("Stream already exists!")
            return stream_id
        self._active_streams[stream_id] = asyncio.create_task(
            self._watch_stream(stream_id=stream_id, out_queue=out_queue)
        )
        return stream_id
    ...
    async def _watch_stream(self, stream_id: str, out_queue: asyncio.Queue):
        backoff = 1
