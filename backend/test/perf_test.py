...
Combines WebSocket stress testing with real-time system resource monitoring.

Usage:
    python perf_test.py                    # Run with defaults (30s, localhost)
    python perf_test.py --duration 60      # Run for 60 seconds
    python perf_test.py --verbose          # Print stream messages
    python perf_test.py --monitor          # Enable live CPU/Memory monitoring

Generated by Claude Code, quadruple checked by @cb3xd
"""
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if isinstance(data, dict):
                ...
                return data
            print("⚠️ Endpoint did not return a dictionary structure.")
            return {}
        ...
        try:
            ...
            if stats.get("verbose"):
                try:
                    ...
                    sys.stdout.write("\r\033[K")
                    print(f"⚠️  [STREAM] non-JSON frame: {msg[:200]}")
...
async def sub_loop(ws, symbol_map, subs_done, stats):
    """Sends subscription payloads sequentially with a 0.15s delay between each."""
    total = len(symbol_map)
    for i, (symbol, exchanges) in enumerate(symbol_map.items(), 1):
        exchange_id = exchanges[0] if exchanges else "binanceusdm"
        payload = {
            "method": "sub",
            "params": {
                "symbol": symbol.lower(),
...
async def single_client_streamer(ws_url, symbol_map, stop_event, subs_done, stats):
    """Maintains a single connection, subscribing and receiving concurrently."""
    connected = False
    try:
        async with websockets.connect(
            ...
            total = len(symbol_map)
            print(
                f"🚀 Connected! Sending {total} subscription payloads (0.15s delay each)..."
            )

            # Run reader and subscriber concurrently so the server is never starved
            await asyncio.gather(
                reader_loop(ws, stop_event, stats),
                sub_loop(ws, symbol_map, subs_done, stats),
            )
        ...
        subs_done.set()  # unblock main even on failure


async def progress_loop(stop_event, subs_done, stats, duration, streams_count, silent):
    """Prints live status during both the subscription phase and the live phase."""
    if silent:
    ...
    while not stop_event.is_set():
        ...
        subs_sent = stats.get("subs_sent", 0)
        status = "ONLINE" if stats["connected"] else "OFFLINE"
        rate = stats["messages_received"] / max(elapsed, 0.01)

        if not subs_done.is_set():
            # Still subscribing — show sub progress + live msg count from already-subbed streams
            sys.stdout.write(
                f"\r⏳ Subscribing {subs_sent}/{streams_count} | "
                f"Msgs so far: {stats['messages_received']} ({rate:.0f}/s) | "
                f"Status: {status}"
            )
        else:
            # All subbed — show normal countdown
...
async def main():
    ...
    parser.add_argument(
        ...
        help="Test run duration in seconds (starts after all subs sent)",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    if not symbol_map:
        print("⚠️ No symbols returned from the API endpoint. Exiting.")
        return
    ...
    if not args.silent:
        est_sub_time = len(symbol_map) * 0.15
        print(
            f"⚡ Subscribing to {len(symbol_map)} streams (~{est_sub_time:.0f}s), then running for {args.duration}s"
        )
        if args.monitor:
    ...
    # Wait for all subs sent, then start the real duration countdown
    await subs_done.wait()
    stats["subs_done_time"] = time.monotonic()
    print(f"\n📡 All subscriptions sent. Running for {args.duration}s...")
